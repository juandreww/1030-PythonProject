...
dataprovince = (dataset.groupby('province')['gmv'].sum().reset_index().sort_values(by = 'gmv', ascending=False).head(5))
dataset['province1st'] = dataset['province'].apply(lambda x: x if(x in dataset['province'].to_list()) else 'other')
dataframejateng = dataset[(dataset['province']=='Jawa Tengah') & (dataset['order_month']>='11-2019')]
datacustomer = (dataframejateng.groupby('customer_id').agg({'order_id':'nunique', 'quantity':'sum', 'gmv':'sum'}).reset_index().rename(columns={'order_id':'orders'}))
print(dataset)
...
dataset_top5brand_dec_per_product['quantity_group'] = dataset_top5brand_dec_per_product['quantity'].apply(lambda x: '>= 100' if x>=100 else '< 100')
dataset_top5brand_dec_per_product.sort_values('quantity',ascending=False,inplace=True)
print(dataset_top5brand_dec_per_product['product_id'])
